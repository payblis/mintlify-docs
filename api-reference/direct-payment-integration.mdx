...
The direct_provider integration uses a secure redirection flow that sends customers directly to the payment provider’s hosted page, where the payment is processed. The customer does not pass through a Payblis checkout, and the result is returned via callback URLs.
